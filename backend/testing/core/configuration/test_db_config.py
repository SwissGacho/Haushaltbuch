...
class TestFileConfig(unittest.TestCase):
    ...
    def _200_create_cfg_searchpaths(self, win=True, mock_loc="mock_location"):
        ...
        with (
            patch("core.configuration.file_config.App") as MockApp,
            patch("core.configuration.file_config.Path", new=MockPath),
            ...
            self.assertEqual(FileConfig._cfg_searchpath, expct_srch)
            self.assertEqual(FileConfig._db_locations, expct_srch[1:3])
    ...
    def test_301_cfg_searchpath(self):
        ...
        FileConfig._cfg_searchpath = mock_paths
        with patch(
            "core.configuration.file_config.FileConfig._create_cfg_searchpaths"
        ) as mock_crt_paths:
            result = FileConfig.cfg_searchpath()
    ...
    def test_401_db_locations(self):
        mock_paths = ["Mock-Paths"]
        FileConfig._db_locations = mock_paths
        with patch(
            "core.configuration.file_config.FileConfig._create_cfg_searchpaths"
        ) as mock_crt_paths:
            result = FileConfig.db_locations()
        mock_crt_paths.assert_not_called()
        self.assertEqual(result, mock_paths)
    ...
    def _600_read_db_config_file(
        ...
        with (
            patch(
                "core.configuration.file_config.FileConfig.cfg_searchpath"
            ) as mock_srch_path,
            patch("core.configuration.file_config.App") as MockApp,
            patch("core.configuration.file_config.Path", new=MockPath),
            patch(
                "core.configuration.file_config.open",
