...
class FileConfiguration(TransientBusinessObject):
    ...
    def __init__(self, cmdline_config: Optional[ConfigDict] = None, **kwargs) -> None:
        ...
        self.file_path = str(FileConfig.db_config_file_path) or ""
    ...
    async def store(self):
        ...
        if self.file_path != str(FileConfig.db_config_file_path):
            LOG.warning(
                f"FileConfiguration.store: File path changed from {FileConfig.db_config_file_path} to {self.file_path}. "
