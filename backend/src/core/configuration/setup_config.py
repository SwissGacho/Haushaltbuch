...
class ConfigSetup(BaseObject):
    ...
    async def _create_or_update_global_configuration(
        ...
        rows_in_db = await ApplicationConfiguration.get_matching_ids()
        if len(rows_in_db) > 1:
            ...
            bo = await ApplicationConfiguration(bo_id=rows_in_db[0]).fetch()
            update_dicts_recursively(target=bo.configuration, source=configuration)
        else:
            bo = ApplicationConfiguration(configuration=configuration)
        # LOG.debug(
